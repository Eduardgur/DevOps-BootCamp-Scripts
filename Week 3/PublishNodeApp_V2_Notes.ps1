...
Write-Output "$WwwDir has been found" -ForegroundColor Green 
...
Write-Output ".env file edited" -ForegroundColor Green
...
# Start-BitsTransfer -Source "https://github.com/jessety/pm2-installer/archive/main.zip" -Description .\
# Expand-Archive -Path ".\main.zip" -DestinationPath ".\"  -Force
# Invoke-Expression -Command "cd .\pm2-installer-main" 
Invoke-Expression -Command "npm install -g pm2" 
Invoke-Expression -Command "npm install -g @innomizetech/pm2-windows-service"
Invoke-Expression -Command $RefEnv
# New-Item -Path "c:\NodeJS" -Name "npm" -ItemType Directory -Force
# New-Item -Path "c:\NodeJS" -Name "npm-cache" -ItemType Directory -Force
# New-Item -Path "c:\NodeJS" -Name "temp" -ItemType Directory -Force
# Invoke-Expression -Command "npm config set prefix 'C:\\NodeJS\\npm'"
# Invoke-Expression -Command "npm config set cache 'C:\\NodeJS\\npm-cache'"
# Invoke-Expression -Command "npm config set temp 'C:\\NodeJS\\temp'"
# Copy-Item "C:\users\${env:username}\.pm2" -Destination "C:\" 
# [Environment]::SetEnvironmentVariable("PM2_HOME", "C:\.pm2", "Machine")
# Invoke-Expression -Command $RefEnv
## Start-Process -FilePath "powershell.exe" -ArgumentList "npm run configure"
## Start-Process -FilePath "powershell.exe" -ArgumentList "npm run configure-policy"
## Start-Process -FilePath "powershell.exe" -ArgumentList "npm run setup"
...
#     -ScriptBlock {"pm2 start $RepoDir\bootstrap.js --name WeightTrackerApp"} `
#     -Credential $VmCred

#Create server restart task
# Write-Output "Registering startup task"
# $TaskName = "NodeJS App"
