...
resource "azurerm_postgresql_server" "server" {
  ...
  ssl_enforcement_enabled = local.server_ssl_enforcment
}

#Create Postgresql server
resource "azurerm_postgresql_database" "database" {
  name                = "${var.name}-${local.db_name_suffix}"
  resource_group_name = var.rg_name
...
resource "azurerm_private_endpoint" "server_private_endpoint" {
  name                = "${var.name}-${local.db_private_endpoint}"
  resource_group_name = var.rg_name
  location            = var.location

  subnet_id = var.server_subnet_id
  private_service_connection {
    name                           = "${var.name}-${local.db_private_service_connection_name_suffix}"
